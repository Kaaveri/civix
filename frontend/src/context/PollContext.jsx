...
export const PollProvider = ({ children }) => {
  ...
  const addComment = useCallback((pollId, text, author) => {
    ...
    const comment = { id: uuidv4(), text, author: author || 'Anonymous', time: new Date().toISOString(), sentiment, isOwner: true };
    setPolls(prev => prev.map(p => p.id === pollId
  ...
  const deleteComment = useCallback((pollId, commentId) => {
    setPolls(prev => prev.map(p => p.id === pollId
  ...
  return (
    <PollContext.Provider value={{
      polls, votedPolls, vote, createPoll, addComment, deleteComment, closePoll, deletePoll,
